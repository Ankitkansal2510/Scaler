...
public class unsetXBitsFromRight {
    public static void main(String[] args) {
        int A=25;
        int B=3;
        int number=unSetBit(A,B);
        System.out.println(number);
        int numberSecondApproach=unSetBit(A,B);
        System.out.println(numberSecondApproach);
    }
    private static int unSetBit(int A,int B){
        return  A &(~0<<B);
    }

    private static int unsetBitSecondApproach(int A,int B){
        int ans=A;
        for(int i=0;i<B;i++){
            if((A&(1<<i))!=0){
                ans=ans-1<<i;
